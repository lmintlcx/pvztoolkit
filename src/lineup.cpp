...
void Lineup::Init(const std::string &string)
{
    ...
    {
        if (plant[i] == 48)
            this->weight += 100000000000000; // Cob Cannon
        if (plant[i] == 43)
            this->weight += 1000000000000; // Gloom Shroom
        if (plant[i] == 42)
            this->weight += 10000000000; // Twin Sun Flower
        if (plant[i] == 45)
            this->weight += 100000000; // Ice Melon
        if (pumpkin[i] == 1)
            this->weight += 1000000; // Pumpkin
        if (base[i] == 1)
            this->weight += 10000; // Water Lilies
        if (base[i] == 2)
            this->weight += 100; // Pots
        if (ladder[i] == 1)
...
bool Lineup::lineup_string_to_data()
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        if (item_type == 16 || item_type == 33) // Water lily pots
        {
        ...
        else if (item_type == 50) // Headstones
        {
            int item_row = atoi(item_str[1].c_str()) - 1;
            int item_col = atoi(item_str[2].c_str()) - 1;
